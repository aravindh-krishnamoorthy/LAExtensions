...
for N in [8, 64, 1000]
    ...
    @btime sqrt($X)
    @btime MatrixAlgorithms.sqrtm($X)
    @btime MatrixAlgorithms.sqrtm($X, MatrixAlgorithms.z_sqrt_quasi_triu!)
    println("$N: Complex/Hermitian")
    @btime sqrt($X+$X')
    @btime MatrixAlgorithms.sqrtm($X+$X')
    @btime MatrixAlgorithms.sqrtm($X+$X', MatrixAlgorithms.z_sqrt_quasi_triu!)
    println("$N: Complex/No structure/PD")
    @btime sqrt($X+$N*I($N))
    @btime MatrixAlgorithms.sqrtm($X+$N*I($N))
    @btime MatrixAlgorithms.sqrtm($X+$N*I($N), MatrixAlgorithms.z_sqrt_quasi_triu!)
    println("$N: Complex/Hermitian/PD")
    @btime sqrt($X+$X'+$N*I($N))
    @btime MatrixAlgorithms.sqrtm($X+$X'+$N*I($N))
    @btime MatrixAlgorithms.sqrtm($X+$X'+$N*I($N), MatrixAlgorithms.z_sqrt_quasi_triu!)
end
